...
Cruzamento da base nacional da Receita Federal (+1.5 milhão de registros) com a base do Cadastur e dados dinâmicos do sistema SIGA.
* **Integração de API Rest:** Extração automatizada de dados em formato JSON com injeção de headers de autenticação.
* **Otimização de Memória (Big Data):** Utilização de processamento em lotes (*Chunking* de 100k linhas) no Pandas para evitar estouro de memória (`MemoryError`) ao ler bases massivas localmente.
* **Data Cleansing:** Aplicação de Expressões Regulares (RegEx) avançadas para padronização de chaves primárias (CNPJ), garantindo a preservação de zeros à esquerda e prevenindo distorções em notação científica.

### 2. Entity Resolution & Fuzzy Matching (Dados Web x Receita Federal)
...
* **Bloqueio Geográfico (Blocking):** Algoritmo otimizado para recortar a base nacional iterativamente por cidade/estado do arquivo em processamento, reduzindo drasticamente a complexidade computacional e o tempo de execução.
...
* **Processamento em Lote (Batch):** Script autônomo ("Trator") capaz de varrer mais de 5.400 arquivos locais, aplicar a inferência léxica e consolidar o resultado em um único *Dataframe Master*.
...
* **Entity Resolution:** thefuzz (`thefuzz[speedup]`)
